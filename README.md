...
사용자가 제시하신 단계별 구성(State, Action, Reward, Agent, Classifier, Phase)과 데이터 이원화(Global Context/Local Patch) 계획은 매우 훌륭합니다. 이 개념을 실제 PyTorch와 MONAI 기반 환경으로 구현할 때 탐색 모드가 로컬 최소값(Local Minima)에 빠지지 않고 안정적인 수렴(Convergence)을 달성할 수 있도록 아래의 **제안 및 보완 사항**들을 반영한 계획을 수립했습니다.

## User Review Required
> [!IMPORTANT]
> 구성된 계획안에는 빠른 탐색 수렴을 위해 사용자 원문 대비 강화된 부분(과거 탐색 기록(RNN) 추가, 연속 행동, IoU 기반 촘촘한 보상, Phase 0 사전학습 병합 등)이 포함되어 있습니다. 내용을 검토해 주시고, 동의하시거나 수정이 필요한 부분이 있다면 편하게 말씀해 주시면 코드 작성 및 환경 구축 작업을 진행하겠습니다.
